...
- `/admin/network/network` and `/admin/network/routes`: modern UCI summaries plus live interface/device status from `dashboard_status`, including protocol, link state, device mapping, addresses, uptime, native common interface/device editing, native static route and policy rule add/edit/remove/duplicate/reorder, and installed LuCI route/rule field coverage. `/admin/network/routes` is supported; the broader network interface view remains partial.
