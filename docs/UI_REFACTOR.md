...
- `/admin/network/firewall` and firewall child routes: modern firewall defaults, zone, zone-forwarding, and traffic-rule editors plus summaries for redirects, with LuCI compat retained for advanced firewall forms.
...
- Firewall is native for defaults editing, zone add/edit/remove, zone forwarding add/edit/remove, traffic rule add/edit/remove for common rule fields, and summary views. Redirect create/edit/delete, custom rule files, nft include handling, advanced zone/rule options, validation, and full save/apply parity remain LuCI compat until the generic form/pending-change adapter is complete.
