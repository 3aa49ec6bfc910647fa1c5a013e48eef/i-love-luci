...
- `/admin/system/package-manager`: clean LuCI compat route in normal use. Adapter evidence exists for parsed installed package inventory, package dependency/detail views, LuCI translation-package filtering, available package browser, available-upgrades planning, manual package-name/URL/staged-file install planning, browser package-file upload staging for `.apk`/`.ipk`, guarded staged-package install with explicit untrusted-signature opt-in, overwrite/autoremove option plumbing, package-feed search, structured APK/OPKG feed editor, package-index update, guarded install/remove plan actions, async job polling for long package operations, and post-mutation HTTP smoke for newly installed LuCI app routes; user routing remains compat until rollback coverage and mutation UX reach parity for broader package workflows.
...
- Console no-reprompt design is currently implemented with `ttyd` bound to LAN, a generated helper HTTP basic-auth credential stored in UCI, and `/bin/login -f root` as the ttyd command. The React header reads safe console metadata on load and opens an internal `#/console?launch=1` route after the user clicks the console button. The console route calls `console_launch`, rotates the ttyd helper password, commits `/etc/config/ttyd`, restarts only `ttyd`, and embeds ttyd in the shell so the user does not need to type router credentials again and the helper credential is not placed in the browser address bar. Security constraints: LAN binding, per-launch random helper credential, existing LuCI session required to request the launch credential, explicit user action before credential exposure, and native audit coverage for helper URL fields and credential rotation.
...
- Package install/remove/update remains LuCI compat by default. Current native package screen has parsed inventory, structured package detail/dependency views, available-upgrades visibility, available feed search, LuCI translation-package suggestions, structured APK/OPKG feed editing, package-index update, manual package-name/URL/staged-file install planning, browser uploaded-package staging and guarded staged-file install, overwrite/autoremove option plumbing, guarded install/remove plan/apply controls, async job/progress polling for long package operations, and post-mutation HTTP smoke for newly installed LuCI app routes. Rollback validation for broader package workflows, URL apply handling, and mutation UX still need coverage before package manager can default to native.
...
| `/admin/system/package-manager` | Package mutation workflow can break router UI. LuCI owns broad install/remove/upgrade behavior, URL install apply, dependency resolution UX, rollback, and post-mutation recovery. | Native adapter evidence covers installed/available package tables, details/dependencies/files, search, feeds editor, i18n suggestions, package index update, manual/staged install planning, async package jobs, and installing/removing `luci-app-example` with route smoke. | Rollback-safe mutation UX, URL install apply parity, broader package install/remove/upgrade matrix, post-mutation app discovery/recovery tests, package-manager error handling, and release-feed install validation. |
...
- Package install-source parity was expanded on `172.16.172.1`: the native package-manager adapter now accepts package names, URL sources, and staged `/tmp/*.apk` or `/tmp/*.ipk` paths for plan-only install checks, and passes overwrite/autoremove options through the bridge. Router validation confirmed `apk add --simulate --force-overwrite jq` plans successfully, staged-file install planning stays non-mutating, URL install planning is accepted, URL apply remains blocked, and `uci changes=0`. Later staged-package install validation covers guarded browser-upload apply; broader mutation UX and rollback validation remain LuCI compat gaps before native promotion.
...
- Have the gateway validate the LuCI session cookie and one-time token before proxying the HTTP/websocket upgrade to localhost-bound ttyd.
