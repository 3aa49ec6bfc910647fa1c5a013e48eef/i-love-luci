...
- Package install/remove/update remains LuCI compat by default. Current native package screen has parsed inventory, structured package detail/dependency views, available-upgrades visibility, available feed search, LuCI translation-package suggestions, structured APK/OPKG feed editing, package-index update, manual package-name/URL/staged-file install planning, overwrite/autoremove option plumbing, guarded install/remove plan/apply controls, and async job/progress polling for long package operations. Browser upload-package install, rollback validation, mutation UX, and broader post-mutation route smoke still need coverage before package manager can default to native.
