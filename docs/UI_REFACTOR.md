...
- Package install/remove/update remains LuCI compat by default. Current native package screen has parsed inventory, available-upgrades visibility, available feed search, structured APK/OPKG feed editing, package-index update, manual package-name/URL/staged-file install planning, overwrite/autoremove option plumbing, and guarded install/remove plan/apply controls. Actual install/remove mutation on the router must use an async job/progress bridge rather than a single browser RPC because package operations can exceed default ubus/rpcd request timeouts; browser upload-package install, translation-package suggestions, dependency detail parity, rollback validation, and post-mutation route smoke still need coverage before package manager can default to native.
...
- `scripts/audit-router-package-manager-mutation.sh` installs and removes `luci-app-example` through the native `package_action` RPC, verifies the newly added routes use LuCI compat, and fails if routes, `/etc/apk/world`, or pending UCI changes are not restored.
...
- Package-manager mutation evidence was expanded on `172.16.172.1`: `scripts/audit-router-package-manager-mutation.sh` installed and removed `luci-app-example` through the native `package_action` RPC, verified the six new `/admin/example*` routes appeared as LuCI compat with no native path, removed the package, restored `/etc/apk/world`, and left `uci_changes=0`. The test uses an extended ubus timeout because real package operations can exceed default request limits; package manager therefore remains LuCI compat until the browser flow has an async job/progress bridge and rollback-safe mutation UX.
