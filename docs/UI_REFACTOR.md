...
- `/admin/services/uhttpd`: uHTTPd service status, lifecycle actions, common and advanced web-server editor, listener/web-server summary, service activity log table, and UCI summary are available as native preview; auto mode defaults to LuCI compat until certificate upload/remove and add/remove-instance parity are complete.
...
- System administration now has native editors for hostname, description, timezone name/POSIX timezone, logging, basic NTP, LuCI language/theme/session/apply rollback timing, router password, Dropbear multi-instance SSH access with direct-interface binding, startup init actions, local startup, cron, SSH authorized keys, LED action add/edit/remove/reorder, uHTTPd common/advanced web-server settings, and uHTTPd certificate defaults. Timezone browser-sync, zram options, and full save/apply parity remain LuCI compat gaps. LED configuration remains partial because LuCI still supports plugin-specific trigger forms. The `luci-app-uhttpd` service route remains partial/compat by default because certificate upload/remove and add/remove-instance parity are not complete.
