...
- Attended sysupgrade configuration parity was expanded on `172.16.172.1`: the native settings editor now includes the LuCI `server.rebuilder` dynamic list while still avoiding image build or flash actions. `/admin/system/attendedsysupgrade/configuration` now routes to the scoped native settings view; root and overview remain LuCI compat. A no-op `attendedsysupgrade_config_save` with the router's current server/client values returned `changed=false`, left `uci changes attendedsysupgrade=0`, and left global `uci changes=0`. Route audit, native page audit, route-mode guard audit, and HTTP route smoke all passed after deploy.
