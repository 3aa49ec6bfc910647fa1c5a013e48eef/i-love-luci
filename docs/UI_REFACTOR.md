...
- `/admin/system/flash`: clean LuCI compat route in normal use. Adapter evidence exists for guarded firmware context, overlay usage, mounted filesystem, flash partition overview, authenticated configuration backup helper check/download, sysupgrade backup file list, and `/etc/sysupgrade.conf` editor. Restore backup upload, factory reset, firmware image validation, flashing progress, reconnect handling, and rollback remain LuCI compat requirements.
...
- Firmware flash destructive actions remain guarded/read-only. Native flash now has structured storage visibility and authenticated configuration backup download, and reboot has a dedicated hostname-confirm RPC. Image upload, sysupgrade validation, flash confirmation, progress reporting, and rollback messaging still need dedicated RPCs.
