...
- `/admin/system/attendedsysupgrade` and children: guarded firmware compatibility context, target metadata, upgrade helper status, native build server/rebuilder/client setting editor, and guardrail messaging. Auto mode defaults to LuCI compat until native image build, progress, package retention behavior, rollback, and flash confirmation reach parity.
- `/admin/system/package-manager`: parsed installed package inventory with package/version/description table and LuCI translation-package filtering, available package browser with LuCI translation-package filtering, available-upgrades table with per-package and all-package upgrade plan actions, manual package install planning, available package-feed search, structured APK/OPKG feed editor, client filter, package family counts, package-index update action, and guarded package install/remove plan/apply actions. Auto mode defaults to LuCI compat until rollback and mutation test coverage reaches parity.
...
- `/admin/system/flash`: guarded firmware, overlay usage, mounted filesystem, flash partition overview, authenticated configuration backup helper check/download, sysupgrade backup file list, and `/etc/sysupgrade.conf` editor. Restore backup upload, factory reset, firmware image validation, flashing progress, reconnect handling, and rollback remain LuCI compat gaps, so this route stays partial.
...
- `/admin/services/banip` and child routes: service status, lifecycle actions, banIP common policy editor, feed/country/interface/log-term list editor, native allowlist/blocklist/custom-feed file editors, runtime file summary, focused reporting/log views, service activity log table, and UCI summary are available as native preview. Because banIP is an installed LuCI app rather than core LuCI, all banIP routes default to LuCI compat unless the route is explicitly forced to modern for adapter testing.
- `/admin/services/adblock-fast`: service status, lifecycle actions, AdBlock Fast common settings editor, advanced DNS/download/cache/update/debug options, allow/block domain list editor, feed-source add/edit/remove/reorder editor, generated dnsmasq server file summary/preview, service activity log table, and UCI summary are available as native preview; auto mode defaults to LuCI compat.
...
- banIP child routes have focused native preview targets for allowlist, blocklist, custom feeds, reporting, firewall log, and processing log when a route is explicitly forced to modern. Auto mode defaults these installed LuCI app routes to compat; native coverage is retained as adapter evidence, not as the default user path. Browser smoke on `172.16.172.1` with the `1.0.0-r4-native36` bundle confirmed `#/native/service/banip/allowlist`, `#/native/service/banip/blocklist`, `#/native/service/banip/feeds`, `#/native/service/banip/setreport`, and `#/native/service/banip/firewall_log` render focused structured views without `<pre>` elements. Route audit, native page audit, and HTTP route smoke all passed after deploy.
...
- Installed LuCI app routes such as banIP and AdBlock Fast remain LuCI compat by default unless an app-specific native migration is explicitly accepted. Native preview pages remain available for adapter testing and include service-specific status summaries, lifecycle controls, structured service logs, whitelisted service file summaries/editors where available, common banIP/AdBlock Fast/uHTTPd editing, and structured UCI-derived tables. banIP report/firewall-log/processing-log workflows still require LuCI compat as the default user path. Dropbear, UPnP, uHTTPd, and custom commands default to native because their installed LuCI workflows are covered and they sit closer to core administration.
...
- Audit every installed LuCI app route, not only top-level app entries, and confirm each route has an explicit I Love LuCI outcome: native route with parity evidence, partial native route with LuCI compat fallback, legacy compat route, or intentionally hidden by ACL/menu metadata.
...
- Package manager preview coverage was expanded on `172.16.172.1`: the native available-upgrades table now exposes plan-only upgrade actions. A simulated upgrade plan for `adblock-fast` returned the expected apk output `(1/1) Upgrading adblock-fast (1.2.2-r18 -> 1.2.4-r2)`, preserved `/etc/apk/world` SHA-256 `23fe9d53009433177208bd4d82687a18cc0457f0d79a75fbd4f463c20ac22c95`, preserved the apk database summary SHA-256 `543615f72125e4b4adec40fb1f792a3731551a120e70286184775726ceee979f`, and left `uci changes=0`. Native upgrade apply remains blocked and LuCI compat remains the default until rollback-safe package mutation parity is proven. Route audit, native page audit, and HTTP route smoke all passed after deploy.
- Manual package planning was added to the native package-manager preview on `172.16.172.1`. A simulated install plan for `jq` returned `(1/1) Installing jq (1.8.1-r2)`, preserved `/etc/apk/world` SHA-256 `23fe9d53009433177208bd4d82687a18cc0457f0d79a75fbd4f463c20ac22c95`, preserved the apk database summary SHA-256 `543615f72125e4b4adec40fb1f792a3731551a120e70286184775726ceee979f`, and left `uci changes=0`. URL/package-file install and package mutation apply remain LuCI compat gaps until secure upload, dependency, rollback, and reboot-safe behavior are proven. Route audit, native page audit, and HTTP route smoke all passed after deploy.
