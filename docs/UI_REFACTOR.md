...
- `/admin/system/package-manager`: parsed installed package inventory with package/version/description table, available-upgrades table with upgrade plan action, available package-feed search, structured APK/OPKG feed editor, client filter, package family counts, package-index update action, and guarded package install/remove plan/apply actions. Auto mode defaults to LuCI compat until rollback and mutation test coverage reaches parity.
