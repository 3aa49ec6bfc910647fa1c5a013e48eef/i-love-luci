...
- `/admin/system/package-manager`: clean LuCI compat route in normal use. Adapter evidence exists for parsed installed package inventory, LuCI translation-package filtering, available package browser, available-upgrades planning, manual package install planning, package-feed search, structured APK/OPKG feed editor, package-index update, and guarded install/remove plan actions; user routing remains compat until rollback and mutation test coverage reaches parity.
...
- Package install/remove/update remains LuCI compat by default. Current native package screen has parsed inventory, available-upgrades visibility, available feed search, structured APK/OPKG feed editing, package-index update, and guarded install/remove plan/apply controls. Actual install/remove mutation on the router, upload-package install, overwrite/autoremove options, translation-package suggestions, dependency detail parity, rollback validation, and post-mutation route smoke still need coverage before package manager can default to native.
