...
- Package install/remove/update remains LuCI compat by default. Current native package screen has parsed inventory, available-upgrades visibility, available feed search, structured APK/OPKG feed editing, package-index update, manual package-name/URL/staged-file install planning, overwrite/autoremove option plumbing, and guarded install/remove plan/apply controls. Actual install/remove mutation on the router, browser upload-package install, translation-package suggestions, dependency detail parity, rollback validation, and post-mutation route smoke still need coverage before package manager can default to native.
...
- Route-mode guard coverage was expanded on `172.16.172.1`: all 16 visible `nativeStatus=compat` routes were tested with `route_mode_set(mode=modern)`, every call returned `saved=false`, and `uci changes=0` remained clean.
...
- Legacy iframe-source smoke coverage was expanded on `172.16.172.1`: all 16 visible compat routes loaded their direct LuCI iframe source URL with `iloveluci_frame=1` without login redirects, dispatch failures, 404s, or HTTP errors.
