...
- `/admin/network/firewall` and firewall child routes: modern firewall defaults, zone, zone-forwarding, traffic-rule, redirect, IP set, whitelisted UCI include section, and custom nftables include file editors, with LuCI compat retained for advanced firewall forms.
...
- Latest router audit on `172.16.172.1` passed with `visible_routes=60`, `modern=47`, `legacy=13`, `native_status supported=31`, `partial=29`, `menu_files=15`, and `luci_apps=9`.
