...
- `/admin/network/network` and `/admin/network/routes`: modern UCI summaries plus live interface/device status from `dashboard_status`, including protocol, link state, device mapping, addresses, uptime, native common interface/device editing, native static route and policy rule add/edit/remove/duplicate/reorder, and installed LuCI route/rule field coverage. Native interface editing covers common LuCI interface options for device/protocol, disable, boot start, force link, default route, IPv4 address/netmask/gateway/broadcast, IPv6 assignment/hint/suffix/prefix classes/routed prefixes, custom DNS, DNS weight, gateway metric, peer DNS, delegation, and DHCP/DHCPv6 client hostname/client-id/vendor/no-release fields. `/admin/network/routes` is supported; the broader network interface view remains partial until add/remove interface, protocol-switch side effects, firewall-zone assignment, and DHCP sub-section parity are proven.
