...
- `/admin/network/dhcp` and `/admin/network/dns`: modern DHCP/DNS surface with dnsmasq/odhcpd service state, active DHCP leases, native DHCP pool add/edit/remove, native static host reservation add/edit/remove, native DNS host record add/edit/remove, and compact UCI summaries. Advanced dnsmasq and odhcpd edit/apply workflows remain LuCI compat.
...
- DHCP/DNS now has native active lease visibility, DHCP pool add/edit/remove, static host reservation add/edit/remove, and DNS host record add/edit/remove. Advanced dnsmasq/odhcpd options, RA flag editing, and full save/apply parity remain LuCI compat until a broader generic form/pending-change adapter is complete.
