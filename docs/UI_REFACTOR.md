...
- `/admin/system/system`: native editor for hostname, description, timezone, logging, basic NTP settings, LuCI language/theme/session/apply rollback timing, and uHTTPd certificate defaults, plus modern summaries for local time, Dropbear, uHTTPd, and LEDs.
...
- `/admin/services/banip` and child routes: service status, lifecycle actions, banIP common policy editor, feed/country/interface/log-term list editor, native allowlist/blocklist/custom-feed file editors, runtime file summary, focused reporting/log views, service activity log table, and UCI summary are available as native preview. The allowlist, blocklist, and custom-feed child routes default to native because edit/reload parity is covered; overview, reporting, and log workflows still default to LuCI compat.
...
- banIP child routes now have focused native targets for allowlist, blocklist, custom feeds, reporting, firewall log, and processing log. Allowlist, blocklist, and custom-feed routes now default to native because edit/reload parity is covered; reporting and log workflows remain LuCI compat. Browser smoke on `172.16.172.1` with the `1.0.0-r4-native36` bundle confirmed `#/native/service/banip/allowlist`, `#/native/service/banip/blocklist`, `#/native/service/banip/feeds`, `#/native/service/banip/setreport`, and `#/native/service/banip/firewall_log` render focused structured views without `<pre>` elements. Route audit, native page audit, and HTTP route smoke all passed after deploy.
...
- banIP allowlist, blocklist, and custom-feed child routes now default to native, while banIP overview, reporting, and logs remain LuCI compat. The `172.16.172.1` deploy passed route audit (`visible_routes=60`, `modern=50`, `legacy=10`, `native_status supported=40`, `partial=20`, `native_preview_routes=60`, `compat_default_routes=10`), native page audit (`core_pages=4`, `native_pages=18`, `service_adapters=6`), and HTTP route smoke (`native_shell_checks=50`, `legacy_route_checks=10`). `banip_file_save` was exercised against the allowlist, blocklist, and custom-feed files with current content; each returned `saved=true`, `changed=false`, the combined file checksum stayed stable, and no pending UCI changes were left.
- Advanced service editors for banIP, AdBlock Fast, and uHTTPd remain LuCI compat by default unless explicit native mode is selected. Native preview pages now include service-specific status summaries, lifecycle controls, structured service logs, whitelisted service file summaries/editors where available, common banIP/AdBlock Fast/uHTTPd editing, and structured UCI-derived tables, but broader native config edit/apply forms should be adapter-based and should land only after pending-change/apply flow is complete. banIP report/firewall-log/processing-log workflows still require LuCI compat. Dropbear, UPnP, and custom commands now default to native because their installed LuCI workflows are covered.
...
- Latest router audit on `172.16.172.1` passed with `visible_routes=60`, `modern=50`, `legacy=10`, `native_status supported=40`, `partial=20`, `menu_files=15`, and `luci_apps=9`.
