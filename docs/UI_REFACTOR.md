...
- `/admin/system/package-manager`: clean LuCI compat route in normal use. Adapter evidence exists for parsed installed package inventory, package dependency/detail views, LuCI translation-package filtering, available package browser, available-upgrades planning, manual package-name/URL/staged-file install planning, browser package-file upload staging for `.apk`/`.ipk`, overwrite/autoremove option plumbing, package-feed search, structured APK/OPKG feed editor, package-index update, guarded install/remove plan actions, async job polling for long package operations, and post-mutation HTTP smoke for newly installed LuCI app routes; user routing remains compat until rollback, browser upload-package install, and mutation UX reach parity.
...
- Package install/remove/update remains LuCI compat by default. Current native package screen has parsed inventory, structured package detail/dependency views, available-upgrades visibility, available feed search, LuCI translation-package suggestions, structured APK/OPKG feed editing, package-index update, manual package-name/URL/staged-file install planning, overwrite/autoremove option plumbing, guarded install/remove plan/apply controls, async job/progress polling for long package operations, and post-mutation HTTP smoke for newly installed LuCI app routes. Browser upload-package install, rollback validation, and mutation UX still need coverage before package manager can default to native.
...
- Package-manager post-mutation smoke was expanded on `172.16.172.1`: `scripts/audit-router-package-manager-mutation.sh` now logs into local uHTTPd with curl while `luci-app-example` is temporarily installed, smokes each newly installed `/admin/example*` route and its `iloveluci_frame=1` iframe source, then removes the package. The audit verified six new routes as LuCI compat, six HTTP/frame smoke checks, successful install/remove jobs, restored `/etc/apk/world`, no remaining `luci-app-example`, no package job leftovers, and `uci_changes=0`. Route audit, native page audit, and full HTTP route smoke passed afterward.
