...
- `/admin/system/attendedsysupgrade` and `/admin/system/attendedsysupgrade/overview`: clean LuCI compat routes in normal use until native image build, long-running ASU server checks, progress, package retention behavior, rollback, and flash confirmation reach parity through a background job/progress RPC. `/admin/system/attendedsysupgrade/configuration` is supported natively because the installed LuCI configuration form is settings-only and its server/client fields are covered and no-op tested.
