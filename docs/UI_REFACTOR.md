...
- `/admin/system/attendedsysupgrade` and children: clean LuCI compat routes in normal use. Adapter evidence exists for guarded firmware compatibility context, target metadata, `owut`/`auc` helper status, native build server/rebuilder/client setting editor, safe local `owut check/list/blob` planning context, package-retention source display, and guardrail messaging. User routing remains compat until native image build, long-running ASU server checks, progress, package retention behavior, rollback, and flash confirmation reach parity through a background job/progress RPC.
...
- Current router checkpoint on `172.16.172.1` after the compat/native UI guard: route audit passed with `visible_routes=57`, `modern=41`, `legacy=16`, `route_status supported=41 compat=16 unsupported=0`, and `native_paths=41`; route-mode guard rejected native mode for all 16 compat routes with `uci_changes=0`; native page audit passed with `core_pages=4`, `native_pages=18`, and `service_adapters=6`; HTTP smoke passed with `native_shell_checks=41`, `legacy_route_checks=16`, and `legacy_frame_checks=16`; future LuCI app audit installed/removed `luci-app-example`, verified six new compat routes, restored `/etc/apk/world`, and left `uci_changes=0`.
