...
- `/admin/network/dhcp` and `/admin/network/dns`: modern DHCP/DNS surface with dnsmasq/odhcpd service state, active DHCP leases, native static host reservation add/edit/remove, native DNS host record add/edit/remove, and compact UCI summaries. DHCP pool, dnsmasq, and odhcpd edit/apply workflows remain LuCI compat.
...
- DHCP/DNS now has native active lease visibility, static host reservation add/edit/remove, and DNS host record add/edit/remove. DHCP pool create/edit/delete, advanced dnsmasq/odhcpd options, and full save/apply parity remain LuCI compat until a broader generic form/pending-change adapter is complete.
