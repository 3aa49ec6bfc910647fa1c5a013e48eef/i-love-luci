...
- `/admin/network/dhcp` and `/admin/network/dns`: modern DHCP/DNS surface with dnsmasq/odhcpd service state, active DHCP leases, native common dnsmasq settings, optional DHCP lease/logging/listen/PXE/TFTP controls, listen interface/address/exclude lists, native DHCP pool add/edit/remove, native static host reservation add/edit/remove, native DNS host record add/edit/remove, and compact UCI summaries. Advanced LuCI relay tables, DNS record-type editors, DNSSEC, IP set binding, tag/class matching, and full PXE boot option tables remain LuCI compat gaps, so these routes stay partial.
