...
- `/admin/network/network` and `/admin/network/routes`: modern UCI summaries plus live interface/device status from `dashboard_status`, including protocol, link state, device mapping, addresses, uptime, native common interface/device add/edit/remove, native static route and policy rule add/edit/remove/duplicate/reorder, and installed LuCI route/rule field coverage. Native interface editing covers common LuCI interface options for device/protocol, existing firewall-zone assignment, disable, boot start, force link, default route, IPv4 address/netmask/gateway/broadcast, IPv6 assignment/hint/suffix/prefix classes/routed prefixes, custom DNS, DNS weight, gateway metric, peer DNS, delegation, and DHCP/DHCPv6 client hostname/client-id/vendor/no-release fields. `/admin/network/routes` is supported; the broader network interface view remains partial until protocol-switch side effects, custom zone creation from the interface form, and DHCP sub-section parity are proven.
...
- Network interface editing now covers additional installed LuCI interface fields: disable interface, bring up on boot, force link, default route, IPv4 broadcast, IPv6 assignment hint/suffix/classes/routed prefixes, custom DNS weight, gateway metric, DHCP hostname/client ID/vendor class, and DHCPv6 no-release. It also supports named interface add/remove with a loopback removal guard and assigning an interface to an existing firewall zone. The `172.16.172.1` deploy compiled the router ucode before install, then passed lint, typecheck, unit tests, production build, route audit (`visible_routes=57`, `modern=41`, `legacy=16`, `native_status supported=41`, `partial=16`, `native_preview_routes=57`, `compat_default_routes=13`, `strict_compat_app_routes=13`, `approved_native_app_routes=11`), native page audit (`core_pages=4`, `native_pages=18`, `service_adapters=6`), and HTTP route smoke (`native_shell_checks=41`, `legacy_route_checks=16`). A temporary disabled `proto none` interface `ilove_tmp_if` was added to firewall zone `lan`, verified in `core_settings(page=network)`, then removed through the native RPC. Final exported `network` checksum stayed `a17a63979cfc26787774abd87acc356d3dbb0ac8a28897541bcf7454a07b8e84`, repeat validation kept exported `firewall` checksum `1e6c58ad03b60347f9c369c2e115c6a4fd15daeb8abd904a8d1bd5b1992fdde7`, and `uci changes=0`. `/admin/network/network` remains partial because LuCI protocol switch side effects, custom firewall-zone creation from the interface form, and DHCP sub-section parity still need adapter coverage before native default promotion.
