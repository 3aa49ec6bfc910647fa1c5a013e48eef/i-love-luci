...
- `/admin/network/firewall` and firewall child routes: modern firewall defaults, zone, and zone-forwarding editors plus summaries for traffic rules and redirects, with LuCI compat retained for advanced firewall forms.
...
- Firewall is native for defaults editing, zone add/edit/remove, zone forwarding add/edit/remove, and summary views. Traffic rule/redirect create/edit/delete, custom rule files, nft include handling, advanced zone options, validation, and full save/apply parity remain LuCI compat until the generic form/pending-change adapter is complete.
