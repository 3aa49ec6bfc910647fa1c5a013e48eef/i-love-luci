...
- The `/admin/system` parent route is marked supported because LuCI treats it as a first-child route to `/admin/system/system`, which now has native parity on the test router. This does not promote partial System children such as flash, attended sysupgrade, or package manager. After deploy, route audit passed with `visible_routes=57`, `modern=39`, `legacy=18`, `native_status supported=39`, and `partial=18`; native page audit and HTTP route smoke both passed.
