...
- System administration now has native editors for hostname, description, timezone name/POSIX timezone, logging, basic NTP, LuCI language/theme/session/apply rollback timing, router password, Dropbear multi-instance SSH access, startup init actions, local startup, cron, SSH authorized keys, LED action add/edit/remove/reorder, uHTTPd common/advanced web-server settings, and uHTTPd certificate defaults. Timezone browser-sync, zram options, and full save/apply parity remain LuCI compat gaps. Dropbear remains partial until full direct-interface option parity is proven. LED configuration remains partial because LuCI still supports plugin-specific trigger forms. The `luci-app-uhttpd` service route remains partial/compat by default because certificate upload/remove and add/remove-instance parity are not complete.
