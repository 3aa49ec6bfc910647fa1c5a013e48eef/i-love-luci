...
- System administration now has native editors for hostname, description, timezone name/POSIX timezone, logging, basic NTP, LuCI language/theme/session/apply rollback timing, router password, Dropbear multi-instance SSH access with direct-interface binding, startup init actions, local startup, cron, SSH authorized keys, LED action add/edit/remove/reorder, uHTTPd multi-instance common/advanced web-server settings, PEM certificate/key upload, guarded certificate removal, and uHTTPd certificate defaults. Timezone browser-sync, zram options, and full save/apply parity remain LuCI compat gaps. LED configuration remains partial because LuCI still supports plugin-specific trigger forms. The `luci-app-uhttpd` service route remains partial/compat by default because binary DER upload and destructive browser remove-flow validation are not complete.
