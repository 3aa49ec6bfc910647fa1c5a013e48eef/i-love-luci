...
| --- | --- | --- | --- |
| `/admin/network`, `/admin/network/network` | Installed LuCI interfaces view includes bridge and `ifname` migration prompts, protocol switching, reconnect/shutdown actions, advanced protocol-specific forms, device advanced options, bridge VLANs, global network options, and optional DSL/ATM sections. | Native page has live status, common interface/device UCI editing, firewall-zone assignment, DHCP pool evidence, static routes, policy rules, and non-mutating status RPCs. | Full protocol-switch behavior, reconnect/shutdown safety, migration prompts, advanced device/bridge/global sections, save/apply rollback handling, and no-op plus add/edit/remove mutation tests. |
