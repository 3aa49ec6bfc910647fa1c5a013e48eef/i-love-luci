...
- `/admin/network/dhcp` and `/admin/network/dns`: modern DHCP/DNS surface with dnsmasq/odhcpd service state, active DHCP leases, native common dnsmasq settings, optional DHCP lease/logging/listen/PXE/TFTP controls, DNS query/filter/cache/port/TTL/file/forwarding controls, upstream server list, resolved-address list, rebind/bogus NXDOMAIN/additional-hosts lists, listen interface/address/exclude lists, native odhcpd global settings, native DHCP relay add/edit/remove, native DHCP pool add/edit/remove, native static host reservation add/edit/remove, native DNS host record add/edit/remove, and compact UCI summaries. Advanced DNS record-type editors, DNSSEC, IP set binding, tag/class matching, IPv6 PXE boot tables, and full PXE boot option tables remain LuCI compat gaps, so these routes stay partial.
