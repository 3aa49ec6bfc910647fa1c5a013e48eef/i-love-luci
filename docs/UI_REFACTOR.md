...
- System administration now has native editors for hostname, description, timezone name/POSIX timezone, logging, basic NTP, LuCI language/theme/session/apply rollback timing, router password, Dropbear multi-instance SSH access with direct-interface binding, startup init actions, local startup, cron, SSH authorized keys, LED action add/edit/remove/reorder, uHTTPd multi-instance common/advanced web-server settings, PEM/DER certificate/key upload, guarded certificate removal, and uHTTPd certificate defaults. Timezone browser-sync, zram options, and full save/apply parity remain LuCI compat gaps. LED configuration remains partial because LuCI still supports plugin-specific trigger forms.
...
- Latest router audit on `172.16.172.1` passed with `visible_routes=60`, `modern=47`, `legacy=13`, `native_status supported=36`, `partial=24`, `menu_files=15`, and `luci_apps=9`.
