...
- `/admin/network/dhcp` and `/admin/network/dns`: modern DHCP/DNS surface with dnsmasq/odhcpd service state, active DHCP leases, native static host reservation add/edit/remove, DNS host records, and compact UCI summaries. DHCP pool, dnsmasq, odhcpd, and DNS record edit/apply workflows remain LuCI compat.
...
- DHCP/DNS now has native active lease visibility and static host reservation add/edit/remove. DNS records, DHCP pool create/edit/delete, advanced dnsmasq/odhcpd options, and full save/apply parity remain LuCI compat until a broader generic form/pending-change adapter is complete.
