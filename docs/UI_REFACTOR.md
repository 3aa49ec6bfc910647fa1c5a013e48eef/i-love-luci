...
- `/admin/system/attendedsysupgrade` and children: clean LuCI compat routes in normal use. Adapter evidence exists for guarded firmware compatibility context, target metadata, upgrade helper status, native build server/rebuilder/client setting editor, and guardrail messaging; user routing remains compat until native image build, progress, package retention behavior, rollback, and flash confirmation reach parity.
