...
- System settings now include additional LuCI `system/system.js` fields: notes, timezone display style, 12/24-hour display, external log server/port/file, NTP server enablement, and NTP server interface binding. The `172.16.172.1` no-op `system_settings_save` returned `changed=false`, kept exported `system` checksum `e3ab589f21098d39cea14ee821c9df83c4f8f5db10ee871c4e718a1408c1a434`, and left `uci changes=0`. The route remains partial until local-time sync buttons, timezone-list selection, and save/apply parity are covered; zram is not present on the router (`luci.getFeatures.zram=false`).
