...
return baseclass.extend({
	...
	render(tree) {
		...
		document.addEventListener('keydown', ui.createHandlerFn(this, 'handleGlobalKeydown'));

		if (!this.sidebarOutsideClickHandler) {
			this.sidebarOutsideClickHandler = ui.createHandlerFn(this, 'handleSidebarOutsideClick');
			document.addEventListener('click', this.sidebarOutsideClickHandler);
		}

		if (!this.pendingChangeHandler) {
			this.pendingChangeHandler = ui.createHandlerFn(this, 'updatePendingChanges');
	...
	handleGlobalKeydown(ev) {
		if (ev.key == '/' && !/^(input|select|textarea)$/i.test(ev.target.tagName || '')) {
